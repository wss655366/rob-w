...
func (d *UserDao) CreateDocDemo() error {
	...
	for i := 0; i < 9; i++ {
		user := model.User{
			...
			Name:     "666666",
			Age:      i,
			Email:    fmt.Sprintf("22%s61@qq.com", RandomMath()),
...
func (d *UserDao) UpsertDocMedo() error {
	user := model.User{
...
func (d *UserDao) UpdateEmbedDocDemo() error {
	...
	// 部分字段
	update = bson.M{"$set": bson.M{"address.province": "beijing", "dodify_at": Now()}}
	err = d.dao.UpdateDoc(d.ColName, selector, update)
	if err != nil {
		return err
	}
...
func (d *UserDao) FindEmbedArrDemo() error {
	...
	selector := bson.M{
		"account":  "mongo_a",
		"comments": bson.M{"$eleMatch": bson.M{"content": "This is a comment"}},
	}
	update := bson.M{"$set": bson.M{ // $ 操作符最后会取代满足 selector 条件的第一个数据元素对应的 index
		"comments.$.email":     "228xx461@qq.com",
		"comments.$.stars":     6,
...
func (d *UserDao) GridFsDemo() error {
	...
	data,err :=ioutil.ReadFile(`E:\caddy\caddy.exe`)
